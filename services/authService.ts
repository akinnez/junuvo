import { LoginCred, LoginResponse } from "@/types/auth";
...
export async function createPasscode(payload: {
  password: string;
  passcode: string;
}): Promise<DataResponse<any>> {
...
export async function updatePasscode(payload: {
  oldPasscode: string;
  newPasscode: string;
}): Promise<Response> {
...
export async function TogglePasscode(
  toggleType: toggleType,
): Promise<DataResponse<any>> {
  const res = await api.post(`/users/auth/toggle-passcode`, toggleType);
  return res.data;
}
export async function LoginPasscode(payload: {
  email: string;
  passcode: string;
}): Promise<DataResponse<any>> {
...
export async function RefreshToken(device: {
  deviceName: string;
  deviceModel: string;
}): Promise<DataResponse<any>> {
  const res = await api.post(`/users/auth/refresh`, device, {
    headers: {
      "x-client-platform": "web",
    },
  });
